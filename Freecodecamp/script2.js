...
d3.json(url).then(function(data) {
  var dataset = data;
  var years = dataset.map(function(item) {return new Date(item.Year, 0);});
  ...
  var time = dataset.map(function(item) {
    return new Date(item.Seconds * 1000);
  }
  );
  var yMax = d3.max(time);
  ...
  svg.selectAll("circle")
    ...
    .append("circle")
    .attr("class", "dot")
    .attr("data-xvalue", function(d) {return new Date(d.Year, 0);})
    .attr("data-yvalue", function(d) {return new Date(d.Seconds * 1000);})
    .attr("cx", function(d) {return xScale(new Date(d.Year, 0));})
    .attr("cy", function(d) {return yScale(new Date(d.Seconds * 1000));})
    .attr("r", 5)
    ...
    .on("mouseover", function(d) {
      d3.select('#tooltip')
        .style("opacity", 1)
        .attr("data-year", function() {return new Date(d.Year, 0);})
        .style("top", (d3.event.pageY - 10) + "px")
        .style("left", (d3.event.pageX + 10) + "px")
        .style("transform", "translate(0, -100%)")
        .html(d.Year+" - "+d.Name+": "+d.Doping)
    })
    ...
    d3.select("#legend")
    .append("text")
    .html("<p><b style='color:green'>■</b> - No doping allegations</p>")
    .attr("x", 250)
    .attr("y", 100)
    .attr("fill", "green")
    ...
    .append("text")
    .html("<p><b style='color:red'>■</b> - Riders with doping allegations</p>")
    .attr("x", 250)
    .attr("y", 100)
    .attr("fill", "red")
